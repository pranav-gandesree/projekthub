...
export default function UserProfile({ username }: { username: string }) {
  ...
  const cancelChanges = () => {
    setEditedUser(user)  // Reset editedUser to original user data
              ...
              <CardContent className="px-6 py-4">
                {isEditing ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-gray-300">{user?.userDetails?.bio || 'No bio available'}</p>
                    <div className="flex flex-col space-y-4 ">
                      {user.userDetails?.twitter && (
                        <a href={user?.userDetails?.twitter} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 flex items-center space-x-4">
                          <TwitterLogoIcon className="w-6 h-6" />
                          <span>{user?.userDetails?.twitter}</span>
                        </a>
                      )}
                      {user.userDetails?.github && (
                        <a href={user.userDetails?.github} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-gray-300 flex items-center space-x-4">
                          <GitHubLogoIcon className="w-6 h-6" />
                          <span>{user?.userDetails?.github}</span>
                        </a>
                      )}
                      {user.userDetails?.portfolio && (
                        <a href={user.userDetails.portfolio} target="_blank" rel="noopener noreferrer" className="text-green-400 hover:text-green-300 flex items-center space-x-4">
                ...
                {isCurrentUser && (
                  <>
                    {isEditing ? (
                      ...
                      <Button onClick={cancelChanges} className="w-full bg-transparent">
                        Cancel
                      </Button>
                      </>
                    ) : (
                      ...
                      <CardTitle className="flex items-center justify-between text-white">
                        ...
                        {project.isPublic ? (
                          ...
                          <Badge variant="outline" className='text-white'>
                            <LockClosedIcon className="w-3 h-3 mr-1 text-white" /> Private
                          </Badge>
                        )}
                      </CardTitle>
