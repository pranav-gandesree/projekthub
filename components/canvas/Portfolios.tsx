    ...
    <div className="min-h-screen bg-background text-foreground">
      <motion.div className="container mx-auto px-4 py-16" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        {Array.isArray(submissions) && submissions.length > 0 ? (
          <motion.div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            {submissions.map((submission, index) => (
              <motion.div key={index} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, delay: index * 0.1 }}>
                <Card>
                  <CardHeader>
                    ...
                    {/* <Link href={`/${submission.uploadedBy}`} className='hover:underline'>
                    <CardTitle className="text-lg font-semibold text-slate-200">
                          {submission?.uploadedBy || 'Anonymous'}
                      </CardTitle>                      
                   </Link> */}

              {submission.uploadedBy &&  submission.uploadedBy !== 'Anonymous' ? (
                <Link href={`/${submission.uploadedBy}`} className='hover:underline'>
                  <CardTitle className="text-lg font-semibold text-slate-200 hover:underline">
                ...
                <CardTitle className="text-lg font-semibold text-slate-200">Anonymous</CardTitle>
                  ...
                  <CardContent className="flex justify-start gap-2">
                    <Button asChild variant="outline" className="w-full">
                      <a href={submission.githubLink} target="_blank" rel="noopener noreferrer">GitHub</a>
                    </Button>
                    <Button asChild variant="outline" className="w-full">
                      <a href={submission.portfolioLink} target="_blank" rel="noopener noreferrer">Portfolio</a>
                    </Button>
                  </CardContent>
                </Card>
              </motion.div>
          ...
          <p className="text-center mt-8">No portfolios submitted yet.</p>
        )}
      </motion.div>
    </div>
  );
